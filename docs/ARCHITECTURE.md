...
`content/curriculum.json` is the phase and task source of truth. `content/resources.json` provides linked learning resources with checked date, access conditions, version notes, availability, and license boundaries. `content/milestones.json` groups phases by observable project outcomes. The app derives the active milestone, next ready core task, prerequisite status, progress, and estimated remaining core hours from these files plus browser state. Recommended and later phases remain available but do not block core progress.
