...
const shareImage = fs.existsSync(path.join(root, 'assets/share-card.png')) ? fs.readFileSync(path.join(root, 'assets/share-card.png')) : Buffer.alloc(0);
...
check(html.includes('property="og:image"') && html.includes('name="twitter:image"') && html.includes('summary_large_image'), 'Missing large social share preview metadata');
check(html.includes('content="https://nevoklein100-png.github.io/doom-learning-os/assets/share-card.png"') && fs.existsSync(path.join(root, 'assets/share-card.svg')), 'Social preview URL or editable SVG source is missing');
check(html.includes('property="og:image:width" content="1200"') && html.includes('property="og:image:height" content="630"'), 'Social preview metadata dimensions do not match the card');
check(shareImage.subarray(0, 8).equals(Buffer.from([137, 80, 78, 71, 13, 10, 26, 10])), 'Missing or invalid PNG social share image');
check(shareImage.length >= 24 && shareImage.readUInt32BE(16) === 1200 && shareImage.readUInt32BE(20) === 630, 'Social share image must be 1200×630');
